    ...
    <div class="flex flex-col">
        <div class="flex flex-col p-2">
            <ul>
                ...
                $semanaActual = \Carbon\Carbon::now()->endOfWeek(); // Obtiene el inicio de la semana actual

                $esSemanaActual = $fecha->isSameWeek($semanaActual); // Verifica si la fecha está en la misma semana que la semana actual
                @endphp

                @if ($esSemanaActual)
                <!-- La fecha corresponde a la semana actual -->
                <p class="text-lg font-semibold  text-white bg-green-600 p-4 rounded-2xl">Estas viendo la semana actual</p>
                <p class="text-3xl text-green-900 font-semibold">Reporte Semanal ({{ \Carbon\Carbon::parse($date)->format('d/m/Y') ?? null }})</p>
        ...
        <div class="">
            <div class="flex items-center justify-between p-2 bg-white dark:bg-gray-800  flex-wrap">
                <div class="">
                    <p class="text-3xl  text-green-800 font-semibold">Historial de Reportes Semanales
